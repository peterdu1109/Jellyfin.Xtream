export default function (view) {
  view.addEventListener("viewshow", () => import(
    ...
    ApiClient.getPluginConfiguration(pluginId).then(function (config) {
      ...
      view.querySelector('#EpgCacheDurationMinutes').value = config.EpgCacheDurationMinutes;
    ...
    const reloadStatus = () => {
      ...
      Xtream.fetchJson('Xtream/TestProvider').then(response => {
        ...
        status.innerText = "Échec. Vérifiez les journaux du serveur.";
        expiry.innerText = "";
        cons.innerText = "";
        maxCons.innerText = "";
        time.innerText = "";
    ...
    view.querySelector('#XtreamCredentialsForm').addEventListener('submit', (e) => {
      ...
      ApiClient.getPluginConfiguration(pluginId).then((config) => {
        ...
        config.EpgCacheDurationMinutes = parseInt(view.querySelector('#EpgCacheDurationMinutes').value, 10);
